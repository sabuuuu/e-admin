...
function Planning() {
  ...
  const getSalles =  () => {
    try {
      axios.get('http://localhost:5555/salles/all', { headers: { 'Authorization': `Bearer ${user.token}` } })
        .then((response) => {
  ...
  return (
    <div className='flex min-h-screen flex-col text-gray-400 bg-gray-900 body-font'>
      ...
      <div className='rounded-xl mb-8 border border-gray-200 shadow  mx-auto  lg:col-span-8 xl:p-6 p-4 md:w-1/2 md:mt-4 justify-center '>
        ...
        <button className='w-full py-2 mt-3 text-white font-body font-semibold bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg' onClick={getPlanning}>Générer</button>
        <div className='my-4'>
          ...
          <button className='w-full py-2 mt-6 mb-6 text-white font-body font-semibold bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg' >Ajouter</button>
      </div>
    </div>
  )
}
